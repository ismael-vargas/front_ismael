...
export class ProductoComponent implements OnInit {
  ...
  getCategorias() {
    this.productoService.funListar().subscribe(  // Cambié funCategoriaProducto por funListar
      (res: any) => {
  ...
  getProductos() {
    this.productoService.funListar().subscribe(  // Cambié funListar2 por funListar
      (res: any) => {
  ...
  guardarProducto() {
    ...
    if (this.producto_id > 0) {
      ...
      this.productoService.funModificar(this.producto_id, productoData).subscribe(  // Cambié funModificar2 por funModificar
        () => {
      ...
      this.productoService.funGuardar(productoData).subscribe(  // Cambié funGuardar2 por funGuardar
        () => {
          this.dialog_visible = false;
          this.getProductos();
  ...
  deleteProduct(prod: Producto) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.productoService.funEliminar(prod.id).subscribe(  // Cambié funEliminar2 por funEliminar
          () => {
